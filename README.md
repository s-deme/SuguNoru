...
「出かける」「帰る」それぞれで、よく使うJR東日本・都営バスの次の便をすぐ確認する個人向け Android アプリです。現在の製品版は `2.1.1` です。

## できること

- 出かける／帰るをタブで切り替え、それぞれに路線を登録
- JR東日本の首都圏在来線と都営バスから、路線・系統、駅・停留所、方面を順に選択
...
登録画面には、JR東日本の首都圏在来線17路線と都営バス32系統の候補ディレクトリを同梱しています。路線・系統、駅・停留所、方面はすべて候補から選び、手入力はしません。この候補は時刻表・運行情報を含まないオフライン用の一覧です。路線改編時はアプリ更新で候補を更新し、保存済みの登録はそのまま利用できます。
